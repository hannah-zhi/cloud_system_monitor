...
- 预警详情:
  ...
  - Keep source pills (`云端预警`, `站端预警`, `站端告警`, `设备告警`) on one line in the source column.
...
- Alarm source options are `云端预警`, `站端预警`, `站端告警`, and `设备告警`; right-panel source counts must show these four categories and update with filters.
...
- Replace risk quick filters with communication quick filters: `通讯正常`, `部分通讯中断`, and `通讯中断`.
- Keep communication quick-filter numeric badges compact and place the whole filter group immediately next to the clear-filter action on the right side of the homepage toolbar.
...
- Topology icons should use refined device-line drawings on a consistent isometric base. Grid towers, step-up station, and wind support must be visually distinct, and each node base center should align with its connection line.
