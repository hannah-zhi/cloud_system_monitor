...
- For stations with communication status `离线`, filter out warning/alarm-class records everywhere they drive central-monitoring state: station-card alarm/fault badges and backgrounds, right-side alarm lists, topology subsystem borders/backgrounds/status, subsystem hover tooltips, and alarm-management table/filter options. Data alarms may remain visible when applicable.
