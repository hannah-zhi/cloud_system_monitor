...
- The two single-station overview chart date windows are independent. Changing `场站有功功率` dates must not update `场站充放电表现`, and changing `场站充放电表现` dates must not update `场站有功功率`; only clamp invalid ranges where start exceeds end within the same chart.
...
- In side-panel list items, device alarms show `告警` in the top-right source pill and use the top-left pill to distinguish `故障` versus `告警`; data items keep the module tag such as `数据系统` on the top left and use `数据` in the top-right pill.
- The source-summary columns may make `数据` narrower and leave more width for `预警` and `告警` so nested subfilter buttons fit. Use yellow for the `告警` subfilter and the list's top-left `告警` tag, matching the `二级` tone. Data list items keep the module tag such as `数据系统` on the top left.
- Prefer low-noise embedded metric styling for side-panel subfilters instead of visually heavy buttons. Keep the same click behavior, but use a right-side indicator column and lightweight active styling such as underline/highlight.
- Side-panel subfilter label and count colors should stay semantic: `一级/故障` red, `二级/告警` yellow. Do not show an extra outline or border on mouse click or focus.
- For side-panel subfilters, color only the label text semantically while counts stay white. Active state should avoid a boxed border; use a subtle row background or left accent line instead.
...
- Side-panel subfilter buttons carry `data-category`; always override generic category active box-shadow/border rules for these nested controls so selected subfilters cannot inherit a boxed outline.

- Detail overview charts use dual axes: station power charts put active/reactive power on the left axis and SOC on the right axis; charge/discharge charts put energy on the left axis and cycle count on the right. Legends should be clickable and highlight, not hide, the selected series.
- Topology subsystem cards use station-card status labels and visual treatment. Fault/alarm backgrounds may change, but warning-level borders remain driven by highest warning level. Tooltips list alarm-class entries before warning-class entries.
- In topology subsystem cards, the subsystem name stays left and the status pill stays at the upper right. Axis titles in dual-axis charts need extra spacing from tick labels.
- Warning-level borders on topology subsystem cards are based only on warning/prewarning records. Device alarm/fault records control background/status label and do not override the border tone.
- Topology subsystem tooltip colors are split by meaning: alarm-class content follows the alarm/fault background tone, while warning-class content follows the warning-level border tone.
- Add a visual fallback so any subsystem card containing an alarm/fault status pill receives the matching alarm/fault background, even if parent class ordering changes.
- Subsystem display names should be formatted as `项目号-#序号子系统`, such as `K0060-#1子系统`, across topology cards, filters, charts, and tables.
- Subsystem status priority is fault, alarm, offline, then normal running state. Tooltip section text and bullets follow their semantic source color: alarm/fault background for alarm-class entries and warning border for warning-class entries.
- Subsystem hover tooltip data should reuse the same base alarm set as the single-station right-side alarm list: visible sources only, current time window, same sorting, and warning/alarm/data grouping.
- Single-station overview chart date inputs are independent start/end selectors. Do not force a fixed window length after one date changes; only clamp invalid ranges where start exceeds end.
- Subsystem hover tooltips always show separate `告警类` and `预警类` sections. The section title and bullet dot use the semantic color, the concrete item names stay white, and empty sections read `当前子系统暂无告警` or `当前子系统暂无预警` instead of using a merged empty state.
- For no-alarm/no-warning subsystem tooltip empty states, use a compact fixed-content layout without an inactive vertical scrollbar.
- Keep empty subsystem tooltip sections visually consistent with populated tooltips: section title, centered empty-state copy, divider line, then the next section.
- Empty subsystem tooltip headings should remain left-aligned like populated tooltip headings, with matching divider width and section spacing.
- For stations with communication status `离线`, filter out only device-alarm-class records (`告警/故障`) wherever they drive central-monitoring state. Keep warning/prewarning records visible and active for right-side lists, warning counts, topology subsystem borders, tooltip warning sections, and alarm-management table/filter options. Data alarms may remain visible when applicable.
- Topology subsystem cards in single-station `场站概览` are navigational controls. Clicking or keyboard-activating a card opens a subsystem detail page; the existing detail shell is reused and the back button should return to the station overview before returning to the station list.
- On the subsystem detail page, keep the right `预警/告警清单` visible and filter it to records whose location contains the selected `#N子系统`, while preserving the same visible-source, time-window, category, and subfilter rules as the station-level list.
- The subsystem detail top cards are `子系统运行` and `充放电统计`; reuse the station overview gauge, status, output, time-switch, and wave-icon visual language rather than introducing a new card style.
- The subsystem detail topology should use independent SVG files from `assets/topology-icons/` for transformer/HV box, PCS, battery-cluster, 环控系统, and 消防系统 icons. Do not draw these device icons with CSS-only pseudo-elements.
- The subsystem detail charts are `子系统有功功率` and `子系统充放电表现`. Reuse the station overview dual-axis chart code paths where possible so date windows stay independent, legends highlight selected series, and hover tooltips show point details.
- The current subsystem topology title is `拓扑图`. Its required structure is transformer to two converter/PCS nodes, then directly to 8-10 battery clusters per converter; avoid adding the old battery-group layer back into the rendered hierarchy.
- Topology icons are interactive filters for the right panel: battery clusters match `RackNNN`, converters match `PCS01`/`PCS02`, and auxiliary icons match 环控/消防 records. Toggle the active icon to clear the part filter, and keep label text visually paired with the corresponding icon.
- In the station `安全诊断` tab, rows in `SOS 安全指数详情清单` should open a subsystem SOS detail page. That page reuses the subsystem topology, shows a warning-only right panel titled `预警清单`, supports the same topology-icon filters, and excludes alarm/data records.
- The subsystem SOS detail content below the topology includes `SOS安全指数与相关数据趋势` plus `#N子系统-各部件SOS安全指数分析`, with charge/discharge energy, SOS trend, and part cards for 电池系统、电气系统、环控系统、消防系统.
- In the refined subsystem topology, connection lines should visually connect to icons, while device text and metrics sit beside the icons. Battery-cluster nodes should not show the `电池簇` text; show the rack number below the icon and keep SOC/SOH aligned on the left.
- In `安全诊断` subsystem SOS bars, axis labels should be compact `#N` values rather than station-prefixed subsystem display names. Continue using label-step logic so labels do not overlap.
- On the subsystem SOS detail page, do not render the left `设备基本信息` table. Put the current SOS index gauge above the right-side `预警清单`, with `更新日期` below it.
- For the warning-only right panel, hide the source summary boxes and use the top `全部 / 一级 / 二级 / 三级` tabs as warning-level filters.
- The subsystem SOS trend chart legend sits below the chart and supports multi-select series with a maximum of five active selections. Keep `充电能量` and `放电能量` as separate series and show selected-series values on hover.
- The part-analysis title should be `各部件SOS安全指数分析`. Risk tags should share pill formatting, healthy is blue, `当前指数` follows the part risk color, and mini trends should expose hover details.
- Keep subsystem topology wire coordinates aligned with icon coordinates. Wires must terminate at transformer, PCS, and rack icons instead of drifting beside or stopping short of them.
- Rack SOC/SOH labels are shared per PCS branch: one label block at the far left of each rack row, with individual rack nodes showing only numeric values.
- In subsystem SOS trend charts, SOS is fixed on the left axis and always visible. Other series are selected through the top-right multi-select control, and legend clicks only highlight/dim the currently visible series.
- Render the current SOS gauge and the warning list as separate right-side cards on the subsystem SOS detail page.
- In the regular subsystem detail top cards, lower the `子系统运行` gauge/text content enough to align visually with the wave icons in `充放电统计`.
- For subsystem SOS trend date controls, keep a dedicated diagnosis start/end date state; changing the date window must redraw the chart, update the X-axis date labels, and auto-scale the right axis for selected non-SOS series.
- Do not show a visible `参数筛选` label beside the trend multi-select; keep the selector directly adjacent to the date range.
- The right-side current SOS gauge card needs title inset and bottom spacing consistent with the warning-list card.
- In the `SOS 安全指数详情清单` table, display compact subsystem labels such as `#1子系统` instead of station-prefixed names.
- Align rack-row `SOC:` / `SOH:` labels to the corresponding numeric rows, and keep regular subsystem running-card content away from the bottom border.
- Station-overview subsystem hover tooltips must be pointer-interactive and remain open when the pointer enters the tooltip, so long alarm/warning lists can be scrolled.
- Keep sample device alarms assigned only to online stations and include visible `告警` examples in station alarm distribution.
- Hide the top `全部 / 一级 / 二级 / 数据` tab row from single-station and single-subsystem right-side `预警/告警清单` panels.
- On the `预警管理` page, exclude `站端预警`, rename list/filter `来源` to `类型`, display cloud warnings as `预警` and device alarms as `告警`, split the two top pies into `告警等级占比` and `预警等级占比`, and remove linked warning/alarm jump buttons from detail cards.
- Use red for `故障` and yellow for `告警` in the alarm-level pie.
- For screenshots and visual validation, set the page to 67% zoom first. Donut canvases should keep their pre-zoom square drawing dimensions so the rings are not clipped and legend spacing stays consistent.
- Alarm-management device-alarm level tags are separate from warning levels: `故障` red, `告警` yellow; empty donut center totals show `0`; detail-card corner ribbons are blue for `预警` and purple for `告警`.
- Alarm-management batch processing starts from right-click multi-select in `预警/告警列表`. Put row checkboxes in a blank column before `编号`, allow batch actions only when all selected groups share the same `状态`, reuse the single-row close modal, and reuse the SR form without the top alarm-context box for batch SR.
- On subsystem diagnosis detail pages, show warning-only tabs `全部 / 一级 / 二级 / 三级` above the right `预警清单`; use `场站功率` as the power-chart title on station and subsystem overview pages; and render only `SOS指数` as a line in `SOS安全指数与相关数据趋势`, with all other selected parameters as grouped bars.
- Right-side `预警/告警清单` and warning-only `预警清单` item cards should show a status tag immediately after the module/system tag, using the same status value and semantic color treatment as the alarm-management table.
- In alarm-management `预警/告警列表`, place a searchable multi-select `编号` filter immediately after the date range; its options must match the grouped table codes and filter to the corresponding grouped rows.

## 2026-06-30 Station-Level Navigation And Right-Panel Cleanup

- Rename the station `场站概览` view to `站级监测` and expose it as a second-level sidebar entry under `全景监测 / 集中监测`.
- Rename the station `安全诊断` view to `站级诊断` and expose it as a second-level sidebar entry under `安全预警 / 预警管理`.
- Clicking a central-monitoring station card opens the selected station directly in `站级监测`.
- Clicking `站级监测` or `站级诊断` from the sidebar with no station selected opens the station with the lowest SOS value by default.
- Both station-level pages need a top single-select station search box. Reuse the central-monitoring station search style, but allow only one selected station and switch the page to that station on selection.
- Hide the old horizontal station-detail tabs for these station-level routes; navigation is owned by the sidebar entries.
- Central-monitoring station cards should not render a `通讯状态` text row. Keep communication state only as the top-right WiFi-style icon.
- The top-right station-card communication icon uses the same semantic colors as the old dot: green normal, amber partial interruption, red interruption, gray offline.
- Right-side lists on all pages should remove data-class alarms from visible category summaries, tabs, counts, and cards. Keep only the warning/alarm classes supported by that page.

## 2026-06-30 Station-Level Visual Polish

- Do not show a duplicated station/project title above the station-level selector. The current station name should live only in the single-select station search box.
- When central-monitoring station-card rows are removed, shorten the fixed card height so the layout does not retain blank vertical space.
- Render the top-right communication indicator as a WiFi-style glyph inside a rounded capsule, with semantic color by communication state.
- When data alarms are removed from right-side panels, lay out the remaining `预警` and `告警` summary boxes as balanced two-column cards.
- Inside each `预警 / 告警` summary box, keep the total block and nested subfilter block as a compact centered group. Do not let the left total block expand to fill all remaining width.

## 2026-06-30 Subsystem Diagnosis Header

- The station-level single-select search box is only for `站级监测` and `站级诊断`.
- Single-subsystem detail/diagnosis pages should hide that selector and show only the compact `场站号-#N子系统` title, such as `K-0239-#4子系统`. Returning to station-level pages should restore the selector.
