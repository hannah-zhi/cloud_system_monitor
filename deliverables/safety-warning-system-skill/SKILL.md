...
- 预警详情:
  ...
  - The 编号 column must preserve continuous row divider lines. Keep the 状态 column wide enough for the SR envelope icon plus complete status pills.
  - The SR envelope belongs to the `状态` column, not the `编号` column.
  ...
  - The modal should include an interactive trend curve for alarm-related data, with `关闭`, `下发 SR`, and `分析` action buttons below the chart; do not use a generic `处理` button.
  - Clicking `关闭` or `下发 SR` opens a separate centered processing modal rather than expanding content at the bottom of the detail modal:
    - `关闭` opens the close form directly, and `下发 SR` opens the SR issue form directly; do not show an intermediate `关闭预警 / 下发 SR` choice step.
    ...
    - After `类型准确/类型不准确` and failure mode are selected, reopening the SR action should show a read-only SR result review instead of the selection form. Only `关闭-待补充根因` shows a root-cause input at the bottom of that result modal.
    - For `关闭-站端已处理` alarms, show station-side action and conclusion inside the closure summary, disable the `关闭` and `下发 SR` buttons, and do not require any additional processing.
